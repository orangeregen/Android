...
public class SecondActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
            ...
            public void onClick(View v) {
                ...
                new Thread(new Runnable() {
                    ...
                    public void run() {
                        ...
                        //String responseBody = "";
                        Request request = new Request.Builder().url("https://api.gameofthronesquotes.xyz/v1/random").get().build();
                        try {
                            ...
                            if (responseCode != 200) {
                                quoteResponse = "The request was invalid";
                            } else {
                                String jsonResponse = response.body().string();
                                quoteResponse = new JSONObject(jsonResponse).getString("sentence").toString();
                            }
                        } catch (IOException e) {
                            //Toast.makeText(getApplicationContext(),"No Internet connection!", Toast.LENGTH_SHORT).show();
                            quoteResponse = "No Internet connection!";

                        } catch (JSONException e) {
                            throw new RuntimeException(e);
                        }

                        String finalNewResponse = quoteResponse;

                        runOnUiThread(new Runnable() {
                            @Override
                            public void run() {
                                myStringArray.add(finalNewResponse);
                                TextAdapter.notifyDataSetChanged();
                            }
                        });
                    }
        ...
        loadData.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                String data = db.getAllUsers().toString();
                String replStr1 = data.replace('[', ' ');
                String replStr2 = replStr1.replace(']', ' ');
                String[] parseData = replStr2.split(",");

                for (String niceData : parseData) {
                    myStringArray.add(niceData);
                    TextAdapter.notifyDataSetChanged();
                }
            }
        });
